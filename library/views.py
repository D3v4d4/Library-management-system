...
def student_change_password(request):
    if not request.user.is_authenticated:
        return redirect('studentlogin')
    """
    Lets a logged-in student change their password without logging them out.
    Uses Django's password validation and keeps the session with update_session_auth_hash.
    """
    if request.method == "POST":
        ...
        # # 3) Run Django validators (strength, similarity, etc.)
        # try:
        ...
        # if request.user.check_password(new1):
        #     messages.error(request, "New password cannot be the same as the current password.")
        #     return redirect("student_change_password")

        # 5) Save and keep the user logged in
...
def admin_change_password(request):
    if not request.user.is_authenticated:
        return redirect('loginadmin')
    """
    Admin changes own password without being logged out.
    Works on request.user (User model). Requires staff/superuser.
    """
